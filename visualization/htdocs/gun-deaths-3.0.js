$(function() {
	...
	var GET_VICTIMS_URL = SERVER_ROOT + 'getVictims.php?fields=victimID,ageGroup,gender,date,isNewtown';
	var GET_MAP_DATA_URL = SERVER_ROOT + 'getMapData.php?callback=?';
	var INCIDENTS_PER_ROW = 40;
	var ACTIVE_MAKER_STYLE = {
		color: '#ED1F24',
		fillOpacity: 1
	};
	var HOVER_MARKER_STYLE = {
		color: '#300018',
		fillOpacity: 1
	};
	var INACTIVE_MARKER_STYLE = {
		color: '#660033',
		fillOpacity: 0.7
	};
	...
	Interactive.prototype = {
		...
		printIncidents: function(incidents) {
			// print victim icons for each incident
			this.clearIncidents();
			this.victims_container.detach(); //detach victims container from DOM for better performance
			var last_printed_data = '2012-12-14';
			var last_printed_phase = 0; //is the incident in an odd day or an even day?
			var incidents_not_newtown = 0;
			var newtown_marker_printed = false;
			var l = incidents.length;
			for (var i = 0; i < l; i += INCIDENTS_PER_ROW) { //for each row
				var victims_row = $('<div class="victim_row">');
				for (var a = 0; a < INCIDENTS_PER_ROW && (a + i) < l; a++) { //for each incident in row
					...
					if (last_printed_data !== incident.prop.date) {
						incident.printDateMarker(a); //a date marker is a little element that shows where each new day begins
				...
				$('<div class="meterCount">') //the meterCounts are the numbers on the righthand side of the interactive that show the incident count thus far
				.html(commaSeparateNumber(i + a))
					.appendTo(victims_row);
				victims_row.appendTo(this.victims_container);
			}
			this.victims_container.appendTo(this.victims_wrapper); //re-attach victims container to DOM
			$('#count_here').html(commaSeparateNumber(incidents_not_newtown));
			return this;
		},
		addEventListeners: function() {
			var self = this;
			this.victims_container.on('click', '.victim', function() {
				if (self.active_victim_obj) {
					self.active_victim_obj.removeClass('active');
				}
				self.active_victim_obj = $(this)
					.addClass('active');
				var victimID = self.active_victim_obj.data('victimid');
				if (self.activeVictim == victimID) {
					...
					self.tooltip.positionIt(self.active_victim_obj);
					var request = self.dataHandler.getDataOnVictim(victimID, function(data) {
						self.tooltip.printData(data[0]);
					});
				}
			});
		...
		loadingSignal: function(on) {
			//show a graphic to indicate data are loading, or hide it
			if (on) {
				$('#loading').show();
			} else {
				$('#loading').hide();
		...
		emptyFilter: function(criterion) {
			//reset a html filter element
			...
			$('#istate').blur(function() {
				...
				if (state.length > 2) {
					if (checkState(state, 'name')) {
						$(this).val(convertState(state, 'abbrev'));
					} else {
						$(this).val('');
					}
				} else {
	...
	Tooltip.prototype = {
		positionIt: function(victim_obj) {
		...
		printData: function(data) {
			...
			$('#btn_reportError')
				.attr('href', 'mailto:slatedata@gmail.com?subject=Info on Incident #' + this.par.active_victim_obj.data('victimid') + ', ' + yyyymmdd_to_mmddyyyy(data.date, true) + ', ' + data.city);
		...
		setMiniMap: function(data) {
			this.marker = new L.Marker([data.lat, data.lng]);
			this.m
				.addLayer(this.marker)
				.setView(new L.LatLng(data.lat, adjustLng(data.lng, this.floater.width() * 0.25, 4)), 4, {
					animate: false
				});
		}
	};
	...
	function Incident(incident_data, lastDate, lastPhase, victims_row, index_in_row) {
		...
		this.prop = {
			victimID: incident_data[0],
			ageGroup: incident_data[1],
			gender: incident_data[2],
			date: incident_data[3],
			isNewtown: incident_data[4]
		};
	...
	Incident.prototype = {
		...
		printDateMarker: function(a, newtown) {
	...
	DataHandler.prototype = {
		getVictimsData: function(criteria, callback) {
			var url = GET_VICTIMS_URL + '&orderBy=date&compact=true' + this.getCriteriaQueries(criteria);
			this.makeRequest(url, callback);
		},
		getDataOnVictim: function(victimID, callback) {
			var url = GET_VICTIMS_URL + ',age,name,city,state,lat,lng,url&categorizations=true&victimID=' + victimID;
			this.makeRequest(url, callback);
		},
		...
		getCriteriaQueries: function(criteria) {
			var s = '';
