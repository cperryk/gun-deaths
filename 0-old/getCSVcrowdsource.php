...
// This script was used to output a CSV document for a Mechanical Turk process.

//connect to database
require_once($_SERVER['DOCUMENT_ROOT'].'/connectToDatabase.php');
$link->select_db('gunDeaths');
$result = $link->query('SELECT victimID,url,name,gender,age,city,state,`date` FROM victims WHERE deleted=0 AND categorization_votes = 0');
